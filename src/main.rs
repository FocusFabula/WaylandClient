...
use libc::{PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP, XDP_PGOFF_RX_RING};
...
// impl Window {
//     fn init(surface_id: u32, width: u32, height: u32) -> std::io::Result<Self> {
//     }

//     fn event_from_frame(&mut self, event_frame: WaylandFrame) -> Option<WaylandEvent> {
//         if event_frame.id == surface_id {

//         }
//     }

//     fn handle_event(event_recive: mpsc::Receiver<WaylandFrame>) -> {
...
impl WaylandConnect {
    ...
    fn new_id(&mut self, object: WaylandObject) -> u32 {
    ...
    fn sync(&mut self) -> std::io::Result<()> {
        ...
        let id = self.new_id(WaylandObject::Callback);
    ...
    fn bind_registry(&mut self) -> std::io::Result<()> {
        ...
        loop {
            if let Some(read_message) = self.read()? {
                if read_message.id == id && read_message.opcode == 0 {
                    self.free_id.push(id);
                    self.objects.remove(&id);
                    break;
                }
                ...
                println!(
                    "id: {} \ninterface {} \nversion: {}",
                    number, interface, version
                );

                match interface.as_str().trim() {
                    "wl_compositor" => {
                        self.wl_compositor = self.new_id(WaylandObject::WlCompositor);
                        println!("comp binding id: {}", number);
                        self.bind_global(number, version, "wl_compositor", self.wl_compositor)?;
                    }
                    "wl_shm" => {
                        self.wl_shm = self.new_id(WaylandObject::wlShm);

                        println!("========= wl_shm binding {}", number);
                        self.bind_global(number, version, "wl_shm", self.wl_shm)?;
                    }
                    "xdg_wm_base" => {
                        self.xdg_wm_base = self.new_id(WaylandObject::XdgWmBase);
                        println!("xdg_wm_base binding id: {}", self.xdg_wm_base);
    ...
    fn wl_surface(&mut self) -> std::io::Result<u32> {
        let id = self.new_id(WaylandObject::WlSurface);
        println!("wl_surface id: {}", id);
    ...
    fn xdg_surface(&mut self, surface_id: u32) -> std::io::Result<u32> {
        let id = self.new_id(WaylandObject::XdgSurface);
    ...
    fn xdg_toplevel(&mut self, xdg_surface: u32) -> std::io::Result<u32> {
        let id = self.new_id(WaylandObject::XdgToplevel);
    ...
    fn get_event(&self, event_frame: WaylandFrame) -> std::io::Result<Option<WaylandEvent>> {
        if let Some(event) = self.objects.get(&event_frame.id) {
            let opcode = event_frame.opcode;
            match (event, opcode) {
                (WaylandObject::XdgToplevel, 0) => {
                    // configurre event
                ...
                (WaylandObject::XdgToplevel, 1) => Ok(Some(WaylandEvent::WindowClose {
                    surface_id: event_frame.id,
                })),
                (WaylandObject::XdgSurface, 0) => {
                    // configure event
                    let mut enc_configure = FrameDecoder::new(event_frame.arguments);
                    let serial = enc_configure.read_uint();

                    Ok(Some(WaylandEvent::ConfigureXdgSurface {
                        xdg_surface_id: event_frame.id,
                        serial,
                    }))
                }
                (_, _) => Ok(None),
    ...
    fn event_pool(
        ...
        if let Some(event_frame) = self.read_frame()? {
            ...
            if self.xdg_wm_base == event_frame.id {
                ...
                let msg_pong = WaylandFrame::new(self.xdg_wm_base, 3, enc_pong.get_buffer());
                self.stream.write_all(&msg_pong.serialize())?;
                self.stream.flush()?;
                return Ok(());
            } else {
    ...
    fn present_window(&mut self, win: &Window) -> std::io::Result<()> {
        let size = (win.width * win.height) as usize * 4;
        // Tworzenie puli SHM (create_pool na obiekcie wl_shm [5])
        self.wl_shm = self.new_id(WaylandObject::wlShm);
        let shm_pool = self.new_id(WaylandObject::wl_shm_pool);
        let mut enc_pool = FrameEncoder::new();
        enc_pool.write_uint(shm_pool); // ID nowej puli = 11
        enc_pool.write_uint(size as u32);
        let serialized = WaylandFrame::new(self.wl_shm, 0, enc_pool.get_buffer()).serialize();
        ...
        unsafe {
            ...
            if !cmsg.is_null() {
                ...
                *fd_ptr = win.buffer.get_fd();
            }

            println!("Wysyłam pulę pamięci SHM przez gniazdo za pomocą libc::sendmsg...");
            let bytes_sent = libc::sendmsg(self.stream.as_raw_fd(), &msg, 0);
        ...
        enc_buf.write_int(win.width as i32);
        enc_buf.write_int(win.height as i32);
        enc_buf.write_int((win.width * 4) as i32); // stride (bajty na linię)
        ...
        self.stream.write_all(&msg_buf.serialize())?;
        ...
        self.stream.write_all(&msg_attach.serialize())?;

        let mut enc_damage = FrameEncoder::new();
        enc_damage.write_int(0);
        enc_damage.write_int(0);
        enc_damage.write_int(win.width as i32);
        enc_damage.write_int(win.height as i32);
        let msg_damage = WaylandFrame::new(WL_SURFACE, 2, enc_damage.get_buffer());
        self.stream.write_all(&msg_damage.serialize())?;
...
struct Window {
    ...
    serial: u32,
    xdg_surface: u32,
    top_level_surface: u32,
}

impl Window {
    fn init(xdg_surface: u32, top_level_surface: u32) -> std::io::Result<Self> {
        let buf = WaylandBuffer::new(0)?;
        Ok(Self {
            buffer: buf,
            width: 0,
            height: 0,
    ...
    fn test(&mut self, width: u32, height: u32) {
        let size = (width * height) as usize * 4;
        self.buffer = WaylandBuffer::new(size).unwrap();

        let pixels = unsafe { std::slice::from_raw_parts_mut(self.buffer.as_mut_ptr(), size) };
    ...
    fn eventHandle(&mut self, event_channel: mpsc::Receiver<WaylandEvent>) {
        if let Ok(event) = event_channel.recv() {
            match (event) {
...
fn main() {
    ...
    println!("=== Binding to global ===");
    test_connection.bind_registry().unwrap();
    println!("=== Bind to wl_surface ===");
    let wl_surf = test_connection.wl_surface().unwrap();
    test_connection.sync();
    println!("=== Create xdg_surface ===");
    let xdg = test_connection.xdg_surface(wl_surf).unwrap();
    test_connection.sync().unwrap();

    println!("=== Create xdg_toplevel ===");
    let top_level = test_connection.xdg_toplevel(xdg).unwrap();
    test_connection.wl_curface_commit(wl_surf);
    test_connection.sync();
    println!("=== Creating a window ===");
    let mut win = Window::init(xdg, top_level);
    println!("=== Creating an event loop");

    let mut (rx, tx) = mpsc::channel();

    thread::spawn(move || {
        test_connection.event_pool(tx).unwrap();
    })
    // // connect to socket
